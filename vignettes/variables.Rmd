...
Each source below is downloaded on demand, which needs network access, so those
chunks are **shown but not executed** when the article is built. Whatever the
source, every variable comes back as a named layer in a `SpatRaster`. To show
what that looks like offline, _envar_ ships a small **real** WorldClim
...
The catalogue below lists every variable available from every source, together
with the `vars = c(...)` codes used to request them.
