...
We use a set of 2971 records of the species in Europe obtained with a search on the **[Global Biodiversity Information Facility](https://www.gbif.org)**, filtering records with a positional uncertainty lower than 1 km. Additionally, we keep only one record among those in the same or adjacent cells, using the _GeoThinneR_ package [@mestretomas2025], to reduce the negative influence of spatial autocorrelation [@boria2014]. The resulting dataset is included in the package assets (`Alps()`) and can be accessed after loading the package in the R session. 
...
# use 'envar' to create a template raster with a 10 km buffer
template <- var_get(shape=occ_sf, buffer = 10, crs=3035) %>% 
            esalandcover(vars="ice")

# create sampling bias raster
dens_ras <- spatialEco::sp.kde(x=occ_sf, ref=template, standardize=TRUE, mask=TRUE, res = res(template))
...
knitr::include_graphics("images/Corr_plot_apollo.png")
```

Tree and meadow cover have a VIF ≥ 3 and are negatively correlated with Pearson’s correlations ≥ |0.6|. Thus, we remove one of these variables (tree).

```{r,results='hide',message =FALSE,warning=FALSE}
# remove tree to avoid correlation issues
predictors$data <- predictors$data[, !names(predictors$data) %in% c("tree")]
...
# row number of the best model (lowest overfitting)
index <- which.min(sdms@results$auc.diff.avg)
sdm_best <- sdms@models[[index]]
```

```{r}
# show tuning table
ordered <- sdms@results[order(sdms@results$auc.diff.avg), ]

# best model metrics
print(ordered[1, c("rm", "fc", "auc.val.avg", "auc.diff.avg", "cbi.val.avg")])
```
...
predictorsAlps <- var_get(shape = Alps, crs = 3035) %>% 
                  chelsa(vars = c("bio1", "bio4", "bio12", "bio15"), years = "1981-2010") %>% 
                  esalandcover(vars = c("meadow", "water", "shannon")) %>% 
                  topography(vars = c("slope", "northness")) %>% 
                  extr_check(calib_points = Apollo, crs = 4326)

# set as NA cells where extrapolation occurs

# predict with the best model over the European Alps
prediction <- dismo::predict(sdm_best, predictorsAlps$data)
```
