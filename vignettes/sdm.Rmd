...
sdms <- ENMeval::ENMevaluate(
  ...
  tune.args = list(fc = c("L", "LQ", "LQH"), rm = 1:8),
  partitions = "user",
  user.grp = block,
...
The exact results of an SDM workflow depend on the versions of the R packages
involved. In particular, some functions used here (e.g. `ENMeval::ENMevaluate()`)
have changed their argument names across releases: older versions expected
`RMvalues` and `fc`, whereas current versions expect a single `tune.args` list (as
used above). To obtain a fully replicable environment with a fixed set of package
versions, we recommend the [`renv`](https://rstudio.github.io/renv/) package.

Starting from a clean project, the following snippet creates a project-local library
and records every package version (including _envar_ and its dependencies) into an
`renv.lock` file:

```{r, eval = FALSE}
# install.packages("renv")
renv::init()        # create a project-local library and an renv.lock file
# ... install/load the packages used in this vignette ...
renv::snapshot()    # write the exact versions to renv.lock
```

Sharing the resulting `renv.lock` lets any user recreate the identical environment with:

```{r, eval = FALSE}
renv::restore()     # install the exact package versions recorded in renv.lock
```

This guarantees that the `ENMeval` (and other) function signatures match the ones
used in this tutorial, avoiding "unused arguments" errors caused by version
mismatches.
