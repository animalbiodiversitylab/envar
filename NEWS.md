...
* `chelsa()` monthly time-series downloads now use CHELSA's current host and
  ...
  beyond 2019.
* `biooracle()` now requires `par_set(res = 5.5)` (Bio-ORACLE's native
  ~0.05° / ~5.5 km resolution) and aborts with a clear message for any other
  value, including the default. `par_set()` now accepts fractional `res`
  multipliers to support this.
