...
Note that the land mask only makes a difference for sources that provide
values over the sea in the first place. Several sources (e.g. the land
cover ones) are land-only by construction and already return NA over the
sea, so applying the mask to them changes nothing. Here we use the mean
annual air temperature (bio1) of the CHELSA 1981-2010 climatology, which
is also interpolated over the sea.
...
processed_nolandmask <- par_set(glacier_region_19 = "Central Europe") %>%
  chelsa(vars = c("bio1"), years = "1981-2010")
```
