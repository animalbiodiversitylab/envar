...
template <- var_get(shape = occ_sf, buffer = 100, crs = 3035) %>% 
            esalandcover(vars = "ice")
...
predictors <- var_get(pointsdf = data[,c("X", "Y")], crs=3035) %>% 
              chelsa(vars = c("bio1", "bio4", "bio12", "bio15"), years =         "1981-2010") %>% 
              esalandcover(vars = c("meadow", "tree", "water", "shannon")) %>% 
...
predictorsAlps <- var_get(shape = Alps, crs = 3035) %>% 
                  chelsa(vars = c("bio1", "bio4", "bio12", "bio15"), years = "1981-2010") %>% 
                  esalandcover(vars = c("tree", "meadow", "water", "shannon")) %>% 
